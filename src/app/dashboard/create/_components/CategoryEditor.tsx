      ...
      {categories.map((cat, idx) => (
        <div key={idx} className="rounded-card border border-border bg-surface">
          ...
          <button
            ...
            className="flex w-full items-center gap-2 px-4 py-3 text-left min-h-[44px] cursor-pointer"
          >
            <span className="font-mono text-xs text-text-muted w-6">{cat.sortOrder}</span>
            ...
            <svg
              className={`w-4 h-4 text-text-muted transition-transform duration-200 ${expandedIdx === idx ? "rotate-180" : ""}`}
