...
export default function JoinPoolPage({ params }: { params: { inviteCode: string } }) {
  ...
  // Clerk-authenticated join (commissioner or existing account holder)
  async function handleClerkJoin() {
    if (!isSignedIn) {
  ...
  async function handleGuestJoin(e: React.FormEvent) {
    ...
    try {
      ...
      if (data.returning && data.hasEntries) {
        // Returning player with picks — show welcome message, then route
        setReturnVisitName(data.displayName);
        setTimeout(() => {
          // If picks are still open, go to picks; otherwise go to leaderboard
      ...
      // New guest or returning without picks — go to picks
      router.push(`/guest-pool/${pool.id}/picks`);
    } catch {
      setError("Something went wrong. Please try again.");
  ...
  // If picks are locked, show leaderboard link instead of join form
  const picksLocked = pool.status !== "OPEN" || new Date(pool.picksDeadline) < new Date();
  ...
  return (
    <div className="mx-auto max-w-md px-4 py-12">
      ...
      {closedReason ? (
        ...
        <Button variant="primary" className="w-full" loading={joining} onClick={handleClerkJoin}>
          Join This Pool
        </Button>
      ) : (
