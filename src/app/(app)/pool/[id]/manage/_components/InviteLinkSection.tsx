...
export function InviteLinkSection({ poolName, tournamentName, inviteCode, inviteUrl, status, picksDeadline, maxEntries, poolType }: InviteLinkSectionProps) {
  ...
  return (
    <div className="rounded-card border-2 border-accent-primary bg-surface p-4">
      <p className="font-sans text-[10px] font-medium text-accent-primary uppercase tracking-[0.5px] mb-2">
        Invite Link
      </p>
      ...
      {emailFeedback && (
        <p className="mt-2 font-sans text-xs text-accent-success bg-[#E8F3ED] rounded-data px-3 py-1.5 text-center">
          {emailFeedback}
        </p>
      )}
      ...
      <div className="grid grid-cols-2 gap-2 mt-3 md:grid-cols-3">
        ...
        <a
          ...
          className="rounded-btn border border-border bg-surface py-2.5 text-center font-sans text-xs font-medium text-text-primary hover:bg-surface-alt transition-colors duration-200 min-h-[44px] flex flex-col items-center justify-center gap-1 cursor-pointer md:hidden"
        >
        ...
        <button
          type="button"
          onClick={handleEmail}
          className="rounded-btn border border-border bg-surface py-2.5 text-center font-sans text-xs font-medium text-text-primary hover:bg-surface-alt transition-colors duration-200 min-h-[44px] flex flex-col items-center justify-center gap-1 cursor-pointer"
        >
          <svg className="h-4 w-4 text-text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
