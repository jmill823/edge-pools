...
export function CategoryEditor({ categories, availableGolfers, onChange }: Props) {
  ...
  // Collect all assigned golfer IDs across all categories
  const assignedIds = new Set(categories.flatMap((c) => c.golfers.map((g) => g.id)));
    ...
    <div className="space-y-3">
      {categories.map((cat, idx) => (
        <div key={idx} className="rounded-lg border border-green-200 bg-white">
          ...
          >
            <span className="text-xs text-green-600 font-mono w-6">
              {cat.sortOrder}
            </span>
          ...
          {expandedIdx === idx && (
            <div className="border-t border-green-100 px-4 py-3 space-y-3">
              ...
              <input
                ...
                value={cat.name}
                onChange={(e) =>
                  updateCategory(idx, { ...cat, name: e.target.value })
                }
              ...
              <div className="space-y-1">
                {cat.golfers.map((g) => (
                  <div
                    key={g.id}
                    className="flex items-center justify-between rounded px-2 py-1.5 text-sm hover:bg-green-50"
                  >
                    <div className="flex items-center gap-2 min-w-0">
                      <span className="text-green-900 truncate">{g.name}</span>
                      {g.country && (
                        <span className="text-xs text-green-500">{g.country}</span>
                      )}
                      {g.owgr && (
                        <span className="text-xs text-green-400">#{g.owgr}</span>
                      )}
                      {/* Show warning if assigned to another category too */}
                      {categories.some(
                        (other, otherIdx) =>
                          otherIdx !== idx &&
                          other.golfers.some((og) => og.id === g.id)
                      ) && (
                        <span className="text-xs text-amber-500" title="Also in another category">
                          ⚠
                        </span>
                      )}
                    </div>
                    <button
                      onClick={() =>
                        updateCategory(idx, {
                          ...cat,
                          golfers: cat.golfers.filter((x) => x.id !== g.id),
                        })
                      }
                      className="ml-2 shrink-0 rounded p-1 text-red-400 hover:bg-red-50 hover:text-red-600"
              ...
              <div className="flex items-center gap-2 pt-2 border-t border-green-100">
                <button
                  ...
                  disabled={idx === 0}
                  className="rounded px-2 py-1 text-xs text-green-700 hover:bg-green-100 disabled:opacity-30"
                >
                  Move Up
                </button>
                <button
      ...
      <button
        onClick={addCategory}
        className="w-full rounded-lg border-2 border-dashed border-green-300 py-3 text-sm font-medium text-green-700 hover:border-green-500 hover:text-green-900"
      >
        + Add Category
      </button>
    </div>
...
function GolferSearch({
  ...
  const filtered = golfers
    .filter(
      (g) =>
        !currentCategoryGolferIds.has(g.id) &&
        ...
        <div className="absolute z-10 mt-1 max-h-48 w-full overflow-y-auto rounded border border-green-200 bg-white shadow-lg">
          {filtered.map((g) => (
            <button
              ...
              className="flex w-full items-center gap-2 px-3 py-2 text-sm text-left hover:bg-green-50"
            >
              ...
              {assignedIds.has(g.id) && (
                <span className="text-xs text-amber-500" title="Already in another category">
                  ⚠
                </span>
              )}
            </button>
